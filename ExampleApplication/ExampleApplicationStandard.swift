...
actor ExampleApplicationStandard: Standard, EnvironmentAccessible, HealthKitConstraint, OnboardingConstraint, AccountStorageConstraint {
    enum ExampleApplicationStandardError: Error {
    ...
    private let logger = Logger(subsystem: "ExampleApplication", category: "Standard")
    
    
    private var userDocumentReference: DocumentReference {
        get async throws {
            guard let details = await account.details else {
                throw ExampleApplicationStandardError.userNotAuthenticatedYet
            }
    ...
    init() {
        if !FeatureFlags.disableFirebase {
            _accountStorage = Dependency(wrappedValue: FirestoreAccountStorage(storeIn: ExampleApplicationStandard.userCollection))
        }
    }
