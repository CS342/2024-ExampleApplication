...
/// A `Scheduler` using the ``ExampleApplicationTaskContext`` to schedule and manage tasks and events in the
/// Example Application.
typealias ExampleApplicationScheduler = Scheduler<ExampleApplicationTaskContext>


extension ExampleApplicationScheduler {
    static var socialSupportTask: SpeziScheduler.Task<ExampleApplicationTaskContext> {
        ...
        return Task(
            ...
            notifications: true,
            context: ExampleApplicationTaskContext.questionnaire(Bundle.main.questionnaire(withName: "SocialSupportQuestionnaire"))
        )
    }

    /// Creates a default instance of the ``ExampleApplicationScheduler`` by scheduling the tasks listed below.
    convenience init() {
